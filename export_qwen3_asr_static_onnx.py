...
def make_dummy_inputs(
    proc, device: torch.device, audio_seconds: float = 30.0
):
    tok = getattr(proc.tokenizer, "audio_token", None) or "<|audio_pad|>"
    sampling_rate = int(
        getattr(getattr(proc, "feature_extractor", None), "sampling_rate", 16000)
        or 16000
    )
    num_samples = max(1, int(round(float(audio_seconds) * sampling_rate)))

    try:
        audio_dummy = np.zeros(num_samples, dtype=np.float32)
        combined = proc(
            text=[tok],
            audio=[audio_dummy],
        ...
        return (
            combined["input_ids"].to(device),
            combined["attention_mask"].to(device),
            combined["input_features"].to(device),
            combined["feature_attention_mask"].to(device),
        )
...
def get_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=30.0,
        help="Static export audio duration in seconds",
    )
    return p.parse_args()
...
def main():
    ...
    input_ids, attention_mask, input_features, feature_attention_mask = make_dummy_inputs(
        proc, device, audio_seconds=args.audio_seconds
    )
    ...
    mel_frames = int(mel_input.shape[1])
    n_audio_tokens = int(a_len[0].item())
    prompt_len = int(input_ids.shape[1])
    print(
        f"[shape] audio_seconds={args.audio_seconds:g} mel_frames={mel_frames} "
        f"audio_tokens={n_audio_tokens} prompt_len={prompt_len}"
    )
    ...
    if n_audio != n_audio_tokens:
        raise RuntimeError(
            f"audio slot mismatch in export sample: slots={n_audio}, expected={n_audio_tokens}"
        )

    audio_features = torch.zeros(
        (B, n_audio, hidden_size), device=device, dtype=torch.float32
    )
    decoder_attention_mask = attention_mask.to(dtype=torch.int64)
    ...
    if args.verify:
        ...
        attention_mask_np = (
            decoder_attention_mask.detach().cpu().numpy().astype(np.int64)
        )
